...
from selenium_worker.pypasser.utils import download_audio, convert_to_wav
from .unified_captchav2_solver import UnifiedCaptchaV2Solver

__all__ = ['reCaptchaV2', 'UnifiedCaptchaV2Solver']
...
class reCaptchaV2(object):
    ...
    def __new__(cls, *args, **kwargs) -> bool:
        ...
        try:
            recaptcha_iframe_element = instance.driver.find_element(By.TAG_NAME, "iframe")
            if recaptcha_iframe_element:
                if 'recaptcha' not in recaptcha_iframe_element.get_attribute('class'):
                    return True
                else:
                    if recaptcha_iframe_element.is_displayed() is False:
                        return True
            else:
                return True
            ...
            if cls.__click_audio_button__(instance.driver):
                print("clicked audio")
                return True
            ...
            while remaining_attempts:
                ...
                print(link)
                file_path = convert_to_wav(download_audio(link, instance.downloads_path))
    ...
    def __init__(self, driver: RemoteWebDriver, play: bool = True, attempts: int = 3, downloads_path: str = ''):
        self.driver = driver
        self.play = play
        self.attempts = attempts
        self.downloads_path = downloads_path

    def __click_check_box__(driver: RemoteWebDriver):
    ...
    def __click_audio_button__(driver: RemoteWebDriver):
    ...
    def __get_audio_link__(driver: RemoteWebDriver, play):
        voice: WebElement

        voice = driver.find_elements(By.TAG_NAME, "iframe")[2]
        for i in driver.find_elements(By.TAG_NAME, "iframe"):
            if 'bframe' in (i.get_attribute('src') or ''):
                voice = i
                break
        driver.switch_to.frame(voice)
        download_btn = WebDriverWait(driver, 10).until(
            EC.any_of(
                EC.presence_of_element_located((By.CSS_SELECTOR, ".rc-audiochallenge-play-button > #audio-source")),
                EC.presence_of_element_located((By.CSS_SELECTOR, ".rc-audiochallenge-tdownload-link"))
            )
        )
        link: str
        if download_btn.tag_name == 'audio':
            link = download_btn.get_attribute('src')
        else: link = download_btn.get_attribute('href')
        if play:
